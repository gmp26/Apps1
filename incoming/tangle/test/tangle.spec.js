describe('adjustable directive', function () {
  var $rootScope, element;
  beforeEach(module('tangle'));
  beforeEach(inject(function(_$compile_, _$rootScope_) {
    $compile = _$compile_;
    $rootScope = _$rootScope_;
    $rootScope.cookies = 4;
    element = $compile('<adjustable number="cookies" min="2" max="100">')($rootScope);
    $rootScope.$digest();
  }));

  it('has a "TKAdjustableNumber" css class', function() {
    expect(element.hasClass('TKAdjustableNumber')).toBe(true);
  });

  it('has a "number" attribute', function() {
    expect(element.attr('number')).toBe("cookies");
  });

  it('copies "number" to scope', function() {
    expect($rootScope.number).toBe("cookies");
  });

  it('has a "min" attribute', function() {
    expect(element.attr('min')).toBe("2");
  });

  it('has a "max" attribute', function() {
